...
namespace CRM_Poli40
{
    public class AddDoctorForm : Form
    {
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            { MessageBox.Show("Фамилия, Имя, Специализация, Логин и Пароль обязательны"); return; }

            try
            {
                using (var conn = Database.OpenConnection())
                {
                    var check = new SqlCommand("SELECT COUNT(*) FROM Doctors WHERE Login = @l", conn);
                    check.Parameters.AddWithValue("@l", txtLogin.Text.Trim());
                    if ((int)check.ExecuteScalar() > 0) { MessageBox.Show("Врач с таким логином уже существует"); return; }

                    string token = Guid.NewGuid().ToString("N");
                    string expiry = DateTime.Now.AddYears(1).ToString("yyyy-MM-dd");
                    string hash = BCrypt.Net.BCrypt.HashPassword(txtPassword.Text);

                    var cmd = new SqlCommand(
                        @"INSERT INTO Doctors (LastName, FirstName, MiddleName, Specialization, Office, Login, PasswordHash, Token, TokenExpiryDate)
                          VALUES (@l, @f, @m, @s, @o, @log, @pwd, @tok, @exp)", conn);
